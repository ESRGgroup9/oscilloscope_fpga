# Runtime Tcl commands to interact with - AXIM_read_xadc_v2_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/AXIM_read_xadc_v2_0_include.tcl
...
# Test: SPS_AXI
# Create a write transaction at sps_axi_addr address
create_hw_axi_txn w_sps_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $sps_axi_addr -data $wdata_1
# Create a read transaction at sps_axi_addr address
create_hw_axi_txn r_sps_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $sps_axi_addr
# Initiate transactions
run_hw_axi r_sps_axi_addr
run_hw_axi w_sps_axi_addr
run_hw_axi r_sps_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_sps_axi_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - SPS_AXI"
} else {
	puts "Data comparison test fail for - SPS_AXI, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: MADC_AXI
set wdata_madc_axi [get_init_data 0]
create_hw_axi_txn w_madc_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_madc_axi}
create_hw_axi_txn r_madc_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_madc_axi_addr
run_hw_axi w_madc_axi_addr
run_hw_axi r_madc_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_madc_axi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - MADC_AXI"
} else {
	puts "Data comparison test fail for - MADC_AXI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
