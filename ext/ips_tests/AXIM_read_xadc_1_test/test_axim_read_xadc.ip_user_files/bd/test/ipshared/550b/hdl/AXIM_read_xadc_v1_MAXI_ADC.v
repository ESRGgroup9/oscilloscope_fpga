...
`timescale 1 ns / 1 ps
	...
	`define XADC_SLV_OFFSET 		32'h0000_0004 // XADC slave address
	`define CONFIG_REG_SLV_OFFSET 	32'h0000_0000 // Config register slave address

	// State Machine states
	parameter [1:0] 
		WAIT = 2'b00, // This state waits for the config_reg to be set
			// meaning that the xadc is already configured 
		READ 			 = 2'b10, // This state reads the xadc value in the slave reg
		//OUT_VAL 		 = 2'b10, // This state outputs the xadc read value and sinalizes
			// it by setting the RX_DONE flag
		WAIT_COMPARE	 = 2'b01; // This state verifies if the xadc have been configured
	  ...
	  always @(posedge M_AXI_ACLK)                                  
	      begin                                                     
	        if (M_AXI_ARESETN == 0)                                
	          ...
	          begin                                                 
	            axi_araddr <= ((mst_exec_state == WAIT) | (mst_exec_state == WAIT_COMPARE)) ? `CONFIG_REG_SLV_OFFSET : `XADC_SLV_OFFSET;            
	          end                                                   
	      end 
	          ...
	          READ:                                                               
	            ...
	            if (read_issued)                                                        
	               ...
	               begin                                                                
	                 ...
	                 if (~axi_arvalid && ~M_AXI_RVALID && ~start_single_read)
	                   begin                                                            
	                     start_single_read <= 1'b1;                                                                          
	                   end                                                              
	                 else if (axi_rready)                                               
