/********************************************************************************/
/*                                                                              */
/*  MtdsDemo3.pde  --  MTDS Library Reference Example 3                         */
/*                                                                              */
/********************************************************************************/
...
/*  SD card and to then render them on the display. In order for this sketch    */
/*  to work correctly, the following bitmaps need to be placed on the SD card.  */
/*  These bitmaps are in the resource zip file downloaded from the Digilent web */
/*  site on the resource page where the sketch was obtained. They should all be */
/*  placed in a folder called Images on the SD card.                            */
/*                                                                              */
/*  Bitmaps files used:                                                         */
/*    Images/IMG_0001.BMP    - zip file or created by running MtdsDemo2         */
/*    Images/IMG_0002.BMP    - zip file or created by running MtdsDemo2         */
/*    Images/IMG_0098.BMP    - zip file or created by running MtdsDemo2         */
/*    Images/IMG_0099.BMP    - zip file or created by running MtdsDemo2         */
/*    Images/IMG_0010.BMP    - in the resource zip file                         */
/*    Images/IMG_0011.BMP    - in the resource zip file                         */
/*    Images/IMG_0100.BMP    - in the resource zip file                         */
/*    Images/IMG_0101.BMP    - in the resource zip file                         */
/*    Images/IMG_0102.BMP    - in the resource zip file                         */
/*    Images/IMG_LOGO.BMP    - in the resource zip file                         */
/*                                                                              */
...
/*  should also be familiar with the operation of MtdsDemo2 as well.            */
/*                                                                              */
/********************************************************************************/
/*  Revision History:                                                           */
/*                                                                              */
/*  2016/09/30(GeneA): created                                                  */
/*                                                                              */
/********************************************************************************/
...
**
**  Description:
**    This function illustrates loading a device independent bitmap into memory
**    and then copying it to the display.
*/
...
bool MtdsTestFs1() {
  ...
  */
  ...
  if (hbmpLoad1 == 0) {
    Serial.println("Error loading IMG_0001.bmp");
    return false;
  }
  mtds.SetDrawingSurface(hdsTest1, hbmpLoad1);

  /* Copy the bitmap loaded from the SD card onto the display. We tile the display
  ** with this bitmap. Note: we are assuming the dimensions of the bitmap here. As
  ** this bitmap was created by a sketch that we wrote and we "know" its dimensions,
  ** this is OK, if not strictly good programming practice.
  ** If we are wrong, and the bitmap that we got isn't really the one created by
  ** MtdsDemo2, nothing bad will happen. If the bitmap is smaller than we are assuming,
  ** BitBlt() will clip the amount copied down to the actual size of the bitmap. If
  ** the bitmap is larger than what we are expecting, we will just get the upper left-hand
  ** corner of whatever the total bitmap contains.
  ** If the bitmap isn't the one we expected, nothing bad will happen, but we also won't
  ...
  */
  ...
  /* Free all resources used before we return so that the memory isn't lost to the
  ** system.
  */
...
bool MtdsTestFs2() {
  ...
  if (hbmpLoad2 == 0) {
    Serial.println("Error loading IMG_0002.bmp");
    return false;
  }
...
**
**  Description:
**    This function loads two bitmaps into memory from device independent bitmap
**    files on the SD card and then copies them to the display. It is essentially
**    identical to the function MtdsTestFs1() except that it loads two bitmaps into
**    memory and copies both to the display.
*/
...
**    This function loads two bitmaps into memory from device independent bitmap
**    files on the SD card and then copies them to the display. It is essentially
**    identical to the function MtdsTestFs3() except for how the bitmap files were
**    created. In this case the two bitmap files are the ones that MtdsDemo2 created
**    by copying the files using the file system API functions.
*/
...
bool MtdsTestFs4() {
  ...
  hbmpLoad1 = mtds.LoadBitmap(szFile1);
  if (hbmpLoad1 == 0) {
    Serial.println("Error loading IMG_0099.bmp");
    return false;
  }
  mtds.SetDrawingSurface(hdsTest1, hbmpLoad1);
  ...
  hbmpLoad2 = mtds.LoadBitmap(szFile2);
  if (hbmpLoad2 == 0) {
    Serial.println("Error loading IMG_0098.bmp");
    return false;
  }
  mtds.SetDrawingSurface(hdsTest2, hbmpLoad2);
...
**  Description:
**    This function loads two bitmaps into memory from device independent bitmap
**    files on the SD card and then copies them to the display. The things that
**    distinguish this function from the previous ones is how the bitmaps were
**    created and what their format is, and that no assumptions are made about their
**    size. This illustrates using GetBitmapDimesions() to determine the size of a
**    bitmap in memory.
**    The images loaded in the previous examples were all created either as saved
**    bitmap files created by the MTDS system, or copies of those files. They are
**    16-bit per pixel images saved using the internal 5:6:5 color format.
**    The bitmap file IMG_0010.BMP was created using the Microsoft Windows Paint program.
**    It is a 16-bit per pixel image stored in the default 5:5:5 color format. The
**    bitmap file IMG_0011.BMP was also created using Windows Paint. It is an 8-bit
**    per pixel image with a color mapping table.
**    The main point of this is to illustrate that the DIB loader in the MTDS display
**    device firmware can handle these DIB formats.
**    The loading of the bitmaps into memory and copying them to the display is the
**    same as was illustrated in the previous examples.
*/
...
bool MtdsTestFs5() {
  ...
  /* Get the DS objects that we are going to need. One for drawing on the display and
  ** the other two for the off-screen bitmaps that we are going to be using.
  */
  hdsDisp = mtds.GetDisplayDs();
  hdsTest1 = mtds.GetDs();
  hdsTest2 = mtds.GetDs();

  /* Load the first bitmap into memory. Here, we are assuming that we know nothing about
  ** the bitmap that was loaded. We call GetBitmapDimensions() to learn what its size is
  ** and what its color format is.
  */
  hbmpLoad1 = mtds.LoadBitmap("Images/IMG_0010.bmp");
  mtds.GetBitmapDimensions(hbmpLoad1, &bmpd1);
  mtds.SetDrawingSurface(hdsTest1, hbmpLoad1);

  /* Load the second bitmap and gets its dimensions.
  */  
  hbmpLoad2 = mtds.LoadBitmap("Images/IMG_0011.bmp");
  mtds.GetBitmapDimensions(hbmpLoad2, &bmpd2);
  mtds.SetDrawingSurface(hdsTest2, hbmpLoad2);

  /* Now we're going to use BitBlt() to copy the bitmaps to the display. Note here, that
  ** rather than having hard coded dimensions for the width and height of the bitmaps,
  we are using the widths and heights that we obtained by calling GetBitmapDimensions().
  */
  ycoCur = 10;
  mtds.BitBlt(hdsDisp, 10,  ycoCur, bmpd1.dxco, bmpd1.dyco, hdsTest1, 0, 0, ropSrcCopy);
  mtds.BitBlt(hdsDisp, 120-bmpd1.dxco/2, ycoCur, bmpd1.dxco, bmpd1.dyco, hdsTest1,
                0, 0, ropSrcCopy);
  mtds.BitBlt(hdsDisp, 229-bmpd1.dxco, ycoCur, bmpd1.dxco, bmpd1.dyco, hdsTest1, 0, 0, ropSrcCopy);
  
  ycoCur += bmpd1.dyco + 10;
  mtds.BitBlt(hdsDisp, 10, ycoCur, bmpd2.dxco, bmpd2.dyco, hdsTest2, 0, 0, ropSrcCopy);
  mtds.BitBlt(hdsDisp, 229-bmpd2.dxco, ycoCur, bmpd2.dxco, bmpd2.dyco, hdsTest2, 0, 0, ropSrcCopy);

  /* Free all of the resources used before we return so that the memory isn't lost to the 
  ** system.
  */
  mtds.DestroyBitmap(hbmpLoad1);
  mtds.DestroyBitmap(hbmpLoad2);
...
**
**  Description:
**    This example illustrates loading bitmaps into memory from files on the SD card
**    and then copying them onto the display. This is similar to the previous examples
**    in this sketch. The differences here are first: how the bitmaps were created and
**    what their pixel formats are, and second, demonstrating reuse of a single DS to 
**    copy multiple bitmaps to the display.
**    The three bitmap files were created using Adobe Photoshop. IMG_0100.BMP is a 16-bit
**    per pixel image in the default DIB 5:5:5 color format. IMG_0101.BMP is a 24-bit per
**    pixel image, and IMG_0102.BMP is a 32-bit per pixel image.
**    In all cases, when a color DIB is loaded into memory, it is converted into a 16-bit
**    per pixel bitmap in the internal 5:6:5 color format.
*/
...
bool MtdsTestFs6() {
  ...
  /* Get the DS objects that we are going to need. One for drawing on the display and
  ** one for the off-screen bitmaps that we are going to be using.
  ** Note, in many previous examples, we have gotten a separate DS to use for each
  ** off-screen bitmap. This example shows reuse of a single DS. In this case, we
  ** call SetDrawingSurface() to select each bitmap into the DS just before calling
  ** BitBlt().
  */
  hdsDisp = mtds.GetDisplayDs();
  hdsTest = mtds.GetDs();
  ...
  */
  hbmpLoad1 = mtds.LoadBitmap("Images/IMG_0100.bmp");
  mtds.GetBitmapDimensions(hbmpLoad1, &bmpd1);

  hbmpLoad2 = mtds.LoadBitmap("Images/IMG_0101.bmp");
  mtds.GetBitmapDimensions(hbmpLoad2, &bmpd2);

  hbmpLoad3 = mtds.LoadBitmap("Images/IMG_0102.bmp");
  ...
  */
  ...
  mtds.BitBlt(hdsDisp, 120-bmpd1.dxco/2, ycoCur, bmpd1.dxco, bmpd1.dyco, hdsTest,
                0, 0, ropSrcCopy);
  
  ycoCur += bmpd1.dyco + 10;
  mtds.SetDrawingSurface(hdsTest, hbmpLoad2);
  mtds.BitBlt(hdsDisp, 120-bmpd2.dxco/2, ycoCur, bmpd2.dxco, bmpd2.dyco, hdsTest,
                0, 0, ropSrcCopy);

  ycoCur += bmpd1.dyco + 10;
  mtds.SetDrawingSurface(hdsTest, hbmpLoad3);
  mtds.BitBlt(hdsDisp, 120-bmpd3.dxco/2, ycoCur, bmpd3.dxco, bmpd3.dyco, hdsTest,
                0, 0, ropSrcCopy);
...
**
**  Description:
**    This example illustrates loading a bitmap into memory from a device independent
**    bitmap file and copying it to the display using BitBlt(). The difference in this
**    case is that the bitmap file contains a bitmap the full size of the display. The
**    bitmap dimensions are assumed, as it was created as a 240x320 pixel image. This
**    also illustrates using the timing functions to determine how long it took to load
**    the bitmap file into memory and how long it took to copy it to the display.
*/
...
bool MtdsTestFs7() {
  ...
  */
  hbmpLoad1 = mtds.LoadBitmap("Images/IMG_LOGO.bmp");
  tusLoad = mtds.GetElapsedTime();
  mtds.SetDrawingSurface(hdsTest1, hbmpLoad1);

  xcoCur = 0;
  ycoCur = 0;
  mtds.BitBlt(hdsDisp, xcoCur, ycoCur, 240, 320, hdsTest1,
                0, 0, ropSrcCopy);
  tusCopy = mtds.GetElapsedTime();
  
  /* Print the load time and the copy time to the serial monitor window.
  */
  Serial.println("MtdsTestFs7 -----");
  Serial.print("Load time = ");
  Serial.print(tusLoad);
  Serial.println(" microseconds");  
  Serial.print("BitBlt time = ");
  Serial.print(tusCopy);
  Serial.println(" microseconds");
  Serial.println("--------");
