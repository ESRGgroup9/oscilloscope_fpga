/********************************************************************************/
/*                                                                              */
/*  MtdsDemo2.pde  --  MTDS Library Reference Example 2                         */
/*                                                                              */
/********************************************************************************/
...
/*  Module Description:                                                         */
/*                                                                              */
/*  This program demonstrates using the MTDS library to render graphhics onto   */
/*  the display and to save and load bitmaps to and from files on an SD card.   */
/*  It also demonstrates using some of the file system API functions to read    */
/*  and write files on the SD card.                                             */
/*                                                                              */
...
void setup() {
  ...
  */
  ...
  /* Mount the file system. We are going to be writing a couple of files to
  ** the SD card, so we must have the file system successfully mounted or we
  ** can't proceed to the rest of the example.
  */  
  ...
  /* The file system code uses the real time clock to set the date and time stamps
  ** on the files that it writes. There is a simple software maintained real time
  ** clock in the display board, but it isn't able to maintain the time when it is
  ** not powered. Each time that the display board resets, the date and time get
  ** set to midnight, January 1, 2016 . Here we are setting it to a differnt time.
  ** Just for the purpose of the example, it is being set to approximately the time
  ** when this demo program was written.
  */
  dtmSet.sec = 0;
  dtmSet.min = 50;
  dtmSet.hour = 13;
...
**
**  Description:
**    This demo illustrates drawing into an offscreen bitmap and then
**    using the SaveBitmap() function to write the offscreen bitmap to
...
bool MtdsTestFs1() {
  ...
  */
  ...
  /* Get and DS and set up for drawing onto an off-screen bitmap.
  */
  hdsTest = mtds.GetDs();
  hbmpTest = mtds.CreateBitmap(40, 40, 16);
  mtds.SetDrawingSurface(hdsTest, hbmpTest);

  /* Draw a test pattern into the off-screen bitmap. Note here that we are using
  ** the drwXorPen drawing mode. This causes the colors to be XOR'd wherever the lines
  ** being drawn cross or overlap.
  */
  ...
  /* Now that we have drawn the test pattern into the off-screen bitmap, copy it
  ** onto the display so that we can see it.
  */
  mtds.BitBlt(hdsDisp,  10,   5, 40, 40, hdsTest, 0, 0, ropSrcCopy);

  /* Here, we come to the main purpose of this demo function. The StoreBitmap() function
  ** writes the off-screen bitmap to a file on the SD card in device independent bitmap
  ** format.
  ** In order to reduce wear on the SD card, we only write the bitmap to the SD card
  ** the first time this function is called. Each subsequent time we get called, we only
  ** draw the test pattern on the off-screen bitmap and then copy it to the display.
  */
  if (fWriteTest1) {
    if (!mtds.StoreBitmap(szFile1, hbmpTest)) {
      Serial.println("StoreBitmap failed on Img_0001.bmp");
      fWriteTest1 = false;
      return false;
    }
  }
  
  /* Now that we have written the bitmap to a file, we're going to clear the off-screen
  ** bitmap, draw a new test pattern and then save that bitmap to a different file.
  */
  ...
  */
  ...
  /* Now, write the off-screen bitmap to the SD card again. Since this is a new pattern
  ** and we don't want to over-write the previous one, we write it to a file with a
  ** different name.
  */
  if (fWriteTest1) {
    if (!mtds.StoreBitmap(szFile2, hbmpTest)) {
      Serial.println("StoreBitmap failed on Img_0002.bmp");
      fWriteTest1 = false;
      return false;
    }
  }

  /* Here, we come to the second main point being illustrated in this function. We are
  ** going to use LoadBitmap() to read the bitmap files from the SD card and then copy
  ** them to the display using BitBlt(). LoadBitmap() reads a device independent bitmap
  ** from a file on the SD card. It creates an offscreen bitmap and then loads the DIB
  ** data from the file, converts it to the internal bitmap format and saves the data
  ** into the offscreen bitmap in memory. We end up with a memory bitmap that is identical
  ** to one created by CreateBitmap() except that is has been initialized with the image
  ** data stored in the DIB file. It can be used in the same way that any other memory bitmap
  ** can be used. It needs to be selected into a DS as the drawing surface. Once that has
  ** been done, it can be drawn on the same as any other memory bitmap and it can be copied
  ** to the display using BitBlt().
  */
  if ((hbmpLoad1 = mtds.LoadBitmap(szFile1)) != 0) {
    mtds.SetDrawingSurface(hdsTest, hbmpLoad1);
    /* Note here that we are kind of cheating by assuming the dimensions of the bitmap
    ** that was obtained from LoadBitmap(). Since we created it originally, we can know
    ** what the dimensions are and use them. However, in general it is better to call
    ** GetBitmapDimensions() on a bitmap obtained from LoadBitmap(). In many cases, that
    ** will be the only way to determine its size.
    */
    mtds.BitBlt(hdsDisp, 100, 140, 40, 40, hdsTest, 0, 0, ropSrcCopy);
  }
  else {
    Serial.println("LoadBitmap failed on IMG_0001");
    return false;
  }

  /* Load the second bitmap file that we created above and copy the resulting bitmap to
  ** the display.
  */
  if ((hbmpLoad2 = mtds.LoadBitmap(szFile2)) != 0) {
    ...
    Serial.println("LoadBitmap failed on IMG_0002.bmp");
    return false;
  }

  /* We must free all resources used in this function before we return so that they aren't
  ** lost to the system.
  */
  ...
  /* Set the control variable so that we only write to the SD card the first time
...
**
**  Description:
**    This demo function illustrates using some of the file system API functions
**    to read and write files. In this case, we take the two bitmap files that
**    were created by the MtdsTestFs1 function and copy them to new files. We then
**    load the bitmaps from these new files into memory and then copy them to the
**    display.
*/
...
bool MtdsTestFs2() {
  ...
  */
  if (fWriteTest2) {
    ...
    /* Copy the first bitmap file into a new bitmap file. We do this by opening
    ** the source file for read and the destination file for write. We read data
    ** from the source file and write it to the destination file until we reach
    ...
    */
    ...
    if ((fhSrc == 0) || (fhDst == 0)) {
      Serial.println("Error opening files to copy bitmap IMP_0001");
      fWriteTest2 = false;
      return false;
    }
    ...
    */
    ...
    if ((fhSrc == 0) || (fhDst == 0)) {
      Serial.println("Error opening files to copy bitmap IMG_0002");
      fWriteTest2 = false;
      return false;
    }
    while (!mtfs.Feof(fhSrc)) {
  ...
  ** to the display. As long as we have correctly copied all of the data from the
  ** source files, the new files will be valid device independent bitmap files and
  ** can be loaded into memory using LoadBitmap().
  */
  
  /* We're going to be drawing on the display, so get the display DS to do it with.
  */
  hdsDisp = mtds.GetDisplayDs();

  /* Load the first of the new bitmap files into memory and set up to copy it to the
  ** display. In order to operate on the new bitmap, we need to obtain a DS and then
  ** set the bitmap we load into memory as the drawing surface.
  */
  ...
  */
  ...
  /* Free all of the resource used before we leave so that the memory isn't lost to
  ** the system.
  */
