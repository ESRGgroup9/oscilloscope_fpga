...
always @(*) begin
	case(state_fsm_calc)
		...
		FSM_CALC_S_DELAY: nstate_fsm_calc = (delayCounter == 2) ? FSM_CALC_S_WRITE : FSM_CALC_S_DELAY;
...
always @(posedge clkWR) begin
	...
	else if(is_state_calc_S_DELAY) begin
		// first_calc 	<= 1'b0;
		// addrWR_r 	<= addrWR_w;
		
		valIndex_r 	<= first_calc ? valIndex_w : valIndex_r;
...
assign addrWR_w   = counterX_WR + (rowWR_w*width);
